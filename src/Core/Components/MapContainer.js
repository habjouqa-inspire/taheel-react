...
const MapContainer = ({
  ...
  title = 'الرجاء إدخال العنوان المتوقع للمركز الجديد',
  newAddress
}) => {
  const [googleConected, setGoogleConected] = useState(false);
  useState(async () => {
    try {
      const queryParams = {
      ...
      console.log("goooooooooooooooooogle mAAAAAAAAP SUCC ")

    } catch (err) {
      console.log("err ", err)
      setGoogleConected(false)
      console.log("goooooooooooooooooogle mAAAAAAAAP ERR ")
  ...
  const getStreetAddrPartsFromGeoResult = (geoResult) => {
    ...
    if (
      addressArray?.filter((x) => x.types[0] === 'country')[0]?.long_name ===
      'السعودية'
    ) {
      setErrMessage('');
      currentAddress = {
      ...
      setField('buildNo', currentAddress.streetNumber.substring(0, 4));
      setField(
        'postalCode',
        `${currentAddress.postalCode}-${currentAddress.postalCodeSuffix}`
      );
      setField('lat', currentAddress.lat);
      setField('lng', currentAddress.lng);
      setField('setCity', !!currentAddress.city);
      setField('setSub', !!currentAddress.area);
      setField('address', {
        caption: currentAddress.address,
        heart: { lat: currentAddress.lat, lng: currentAddress.lng }
      });
    } else {
      setErrMessage('الرجاء اختيار موقع داخل حدود المملكة العربية السعودية');
      setStreetAddr(currentAddress);
      setField('sub', null);
      setField('city', null);
      setField('street', null);
      setField('buildNo', null);
      setField('postalCode', null);
      setField('lat', null);
      setField('lng', null);
    }
    ...
    <Grid item md={12} xs={12}>
      {newAddress ? (
        <Typography
          ...
          component="p"
        >
          الرجاء إدخال العنوان الجديد للمركز
        </Typography>
      ) : (
        <Typography
          gutterBottom
