...
import { getMapViewportFromAreas, validArea, validLocation } from './utils'
...
class HeatMapAddress extends Component {
  constructor(props) {
    super(props)
    ...
    this.state = {
      ...
      showPinDrop: props.showPinDrop,
      showMap: false,
      mapFirstShowing: true,
      currentLocation: null,
      pauseMarker: props.pauseMarker,
      //setErrMessage: (errMessage) => props?.setErrMessage(errMessage),
    }
  }
  ...
  getStateFromProps = async props => {
    ...
    if (isEmpty(address) && !isEmpty(markerPosition)) {
      ...
      return props.getStreetAddrPartsFromGeoResultMine(geoResp.results[0])
    }
  ...
  handleMarkerDragEnd = async event => {
    if (this.props.pauseMarker) {
      return;
    }
    var newLat = event.latLng.lat()
    var newLng = event.latLng.lng()
    if ((newLng >= 34.587349601439556 && newLng <= 54.76399716827487) && (newLat >= 17.36831977105389 && newLat <= 32.13436521258608)) {
      this.setState({
        errMessage: "",
        updateMapPos: true
      })
    ...
    if (this.state.updateMapPos) {
      const geoCodeResp = await Geocode.fromLatLng(newLat, newLng)
      geoCodeResp.results = geoCodeResp.results.filter(r => r.formatted_address.includes(this.props.placeAddress))
      ...
      if (this.props.areaMode === false) {
        ...
        if (!!geoCodeResp.results[0]) {
          this.setState({
            ...
            ...this.props.getStreetAddrPartsFromGeoResultMine(geoCodeResp.results[0]),
          })
        } else {
          this.setState({
            errMessage: "الموقع غير معروف",
            updateMapPos: false,
            ...this.props.getStreetAddrPartsFromGeoResultMine(null),
            ...({}),
          })
        }
        return
  ...
  saveInStateStreetAddressDetails = (newLat, newLng, geoResult, updateMapPos) => {
    ...
    this.setState({
      ...this.props.getStreetAddrPartsFromGeoResultMine(geoResult),
  ...
  getUserAreaOptionsFromGeocodeResponse = (placeAddress, geoResp) => {
    ...
    let minLevel = 0
    if (placeAddress) {
      const exactResult = geoResp.results.find(r => r.formatted_address.includes(placeAddress))
      if (exactResult) {
  ...
  handleAddNewArea = async () => {
    ...
    if (!isEmpty(currentLocation)) {
      ...
      areaOptionsFromCurrentLocation = {
        userAreaOptions: this.getUserAreaOptionsFromGeocodeResponse('سعودية', geoCodeResp),
      }
    }
    this.setState({
      ...currentLocationSetting,
  ...
  render() {
    const { classes, inputComponent, inputProps, boundaries, areaMode, placeAddress,mapCenterPosition } = this.props
      ...
      <div>
        ...
        { // showMap && odai
            ...
            <HeatMapWindow
              ...
              pauseMarker={this.props.pauseMarker}
...
export default withStyles(styles)(withGmapsContext(HeatMapAddress))
